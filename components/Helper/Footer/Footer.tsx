...
export default function Footer({ footerData }: FooterProps) {
  ...
  return (
    <footer className="bg-[#18212b]">
      <div className="pt-8 md:pt-10 lg:pt-14">
        {/* Main Footer */}
        <div className="container">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-6 sm:gap-8 lg:gap-10 pb-8 md:pb-10 lg:pb-14">
            {/* Logo */}
            <div className="col-span-1 flex flex-col sm:items-start items-center text-center sm:text-left">
              <Link href="/">
                <Image
                  src={logoUrl}
              ...
              <p className="text-gray-300 text-sm mt-4 max-w-sm">
                ...
                {footerData.navitems.map((item) => (
                  <li key={item.id}>
                    <Link
                      ...
                      className="text-sm leading-snug break-words hover:text-[#ff6100] transition-colors text-[rgb(179,191,204)]"
                    >
                      {item.label}
                    </Link>
                  </li>
                ...
                {footerData.companyitems.map((item) => (
                  <li key={item.id}>
                    <Link
                      href={item.href}
                      target={item.isExternal ? "_blank" : "_self"}
                      className="text-sm hover:text-[#ff6100] transition-colors text-[rgb(179,191,204)]"
                    >
                      {item.label}
                    </Link>
                  </li>
            ...
            <div className="col-span-1">
              ...
              <div className="space-y-3 sm:space-y-4">
                <div className="flex flex-col flex-wrap gap-3 leading-5">
                  {footerData.contactLinks.map((item) => (
                    <Link
                      key={item.id}
                      href={item.href}
                      target={item.isExternal ? "_blank" : "_self"}
                      className="flex items-start gap-3 text-white text-xs sm:text-sm break-all"
                    >
                      {item.image?.url && (
                        <Image
                      ...
                      <span className="hover:text-[#ff6100]">{item.label}</span>
                    </Link>
                  ))}
                </div>
        ...
        <div className="border-t border-[rgba(179,191,204,0.2)] py-4 sm:py-6">
          <div className="container">
            <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between text-center md:text-left">
              ...
              {/* Copyright */}
              <p className="text-[10px] sm:text-xs text-[rgba(179,191,204,0.6)] text-center md:text-right">
